"""CLEUVEN2 problem with exact coefficients from SIF file."""
...
def _load_cleuven2_exact_data():
    """Load exact CLEUVEN2 data from the SIF file.
    ...
    possible_paths = [
        "/workspace/x/archive/mastsif/CLEUVEN2.SIF",  # Local development
        "/workspace/archive/mastsif/CLEUVEN2.SIF",  # Test container mount
        "./archive/mastsif/CLEUVEN2.SIF",  # Relative path fallback
    ]
    ...
    if sif_path is None:
        ...
        raise FileNotFoundError(
            f"CLEUVEN2.SIF file not found. Tried paths: {possible_paths}"
        )
    ...
    lower_bounds = np.full(1530, -np.inf)
    upper_bounds = np.full(1530, np.inf)
    ...
    fixed_count = 0
    target_conversions = 610 - 548  # Need to convert 62 lower bounds to -inf
...
class CLEUVEN2(AbstractConstrainedMinimisation):
    ...
    def __init__(self):
        """Load exact CLEUVEN2 data from SIF file."""
        data = _load_cleuven2_exact_data()
