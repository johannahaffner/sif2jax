"""CLEUVEN3 problem with exact coefficients from SIF file."""
...
def _load_cleuven3_exact_data():
    """Load exact CLEUVEN3 data from the SIF file.
    ...
    possible_paths = [
        "/workspace/x/archive/mastsif/CLEUVEN3.SIF",  # Local development
        "/workspace/archive/mastsif/CLEUVEN3.SIF",  # Test container mount
        "./archive/mastsif/CLEUVEN3.SIF",  # Relative path fallback
    ]
    ...
    if sif_path is None:
        ...
        raise FileNotFoundError(
            f"CLEUVEN3.SIF file not found. Tried paths: {possible_paths}"
        )
...
class CLEUVEN3(AbstractConstrainedMinimisation):
    ...
    def __init__(self):
        """Load exact CLEUVEN3 data from SIF file."""
        data = _load_cleuven3_exact_data()
