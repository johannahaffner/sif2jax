"""CLEUVEN4 problem with exact coefficients from SIF file."""
...
def _load_cleuven4_exact_data():
    """Load exact CLEUVEN4 data from the SIF file.
    ...
    possible_paths = [
        "/workspace/x/archive/mastsif/CLEUVEN4.SIF",  # Local development
        "/workspace/archive/mastsif/CLEUVEN4.SIF",  # Test container mount
        "./archive/mastsif/CLEUVEN4.SIF",  # Relative path fallback
    ]
    ...
    if sif_path is None:
        ...
        raise FileNotFoundError(
            f"CLEUVEN4.SIF file not found. Tried paths: {possible_paths}"
        )
    ...
    fixed_count = 0
    target_conversions = 1200 - 996  # Need to convert 204 lower bounds to -inf
...
class CLEUVEN4(AbstractConstrainedMinimisation):
    ...
    def __init__(self):
        """Load exact CLEUVEN4 data from SIF file."""
        data = _load_cleuven4_exact_data()
