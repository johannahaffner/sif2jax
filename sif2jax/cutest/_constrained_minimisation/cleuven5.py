"""CLEUVEN5 problem with exact coefficients from SIF file."""
...
def _load_cleuven5_exact_data():
    """Load exact CLEUVEN5 data from the SIF file.
    ...
    possible_paths = [
        "/workspace/x/archive/mastsif/CLEUVEN5.SIF",  # Local development
        "/workspace/archive/mastsif/CLEUVEN5.SIF",  # Test container mount
        "./archive/mastsif/CLEUVEN5.SIF",  # Relative path fallback
    ]
    ...
    if sif_path is None:
        ...
        raise FileNotFoundError(
            f"CLEUVEN5.SIF file not found. Tried paths: {possible_paths}"
        )
...
class CLEUVEN5(AbstractConstrainedMinimisation):
    ...
    def __init__(self):
        """Load exact CLEUVEN5 data from SIF file."""
        data = _load_cleuven5_exact_data()
