...
class _LanczosBase(AbstractUnconstrainedMinimisation, strict=True):
...
class LANCZOS1LS(_LanczosBase, strict=True):
    ...
    # Dependent variable values (y) - specific to LANCZOS1
    y_values: jnp.ndarray = None  # Will be computed in __post_init__
    ...
    def __post_init__(self):
        ...
        self.y_values = jax.vmap(lambda x: self.model(x, exact_params))(self.x_values)
...
# TODO: Human review needed to verify the implementation matches the problem definition
class LANCZOS2LS(_LanczosBase, strict=True):
